...
/**
 * pint_push - Runs functions given to binary tree node,
 *             sents it's children into levelorder_queue_t queue
 * @node: Binary tree node to print and push
 * @head: Pointer head of queue
 * @tail: Pointer tail of queue
 * @func: Pointer function call on @node
 *
 * Description: If malloc fails, exits with status code 1
 */
void pint_push(binary_tree_t *node, levelorder_queue_t *head,
		levelorder_queue_t **tail, void (*func)(int))
{
 ...
 * @head: Double pointer to head queue
 */
void pop(levelorder_queue_t **head)
{
	levelorder_queue_t *tmp;
 ...
 * @tree: Pointer to root node tree to traverse
